...
omitted_sources = ["mod_xlsx.c", "mod_csv_reader.c", "readstat.c"]
...
source_dirs = [
    dirname
    for dirname, _, _ in os.walk(source_dir_root)
    if dirname not in omitted_source_dirs
]
...
if os.name == "nt":
    ...
    if os.path.isfile("win_libs/64bit/libiconv-static.lib") and os.path.isfile(
        "win_libs/64bit/libz-static.lib"
    ):
...
extensions = cythonize(
    extensions, compile_time_env={"PY_MAJOR_VERSION": PY_MAJOR_VERSION}, force=True
)
