...
class TestBasic(unittest.TestCase):
    ...
    def _prepare_data(self):
        ...
        df_pandas["mydate"] = [
            datetime.strptime(x, "%Y-%m-%d").date() if type(x) == str else float("nan")
            for x in df_pandas["mydate"]
        ]
        df_pandas["dtime"] = [
            datetime.strptime(x, "%Y-%m-%dT%H:%M:%S.000000")
            if type(x) == str
            else float("nan")
            for x in df_pandas["dtime"]
        ]
        df_pandas["mytime"] = [
            datetime.strptime(x, "%H:%M:%S.000000").time()
            if type(x) == str
            else float("nan")
            for x in df_pandas["mytime"]
        ]
        ...
        df_pandas_formatted["mylabl"] = df_pandas_formatted["mylabl"].apply(
            lambda x: mylabl_format[x]
        )
        df_pandas_formatted["myord"] = df_pandas_formatted["myord"].apply(
            lambda x: myord_format[x]
        )
        ...
        self.df_sas_dates2 = pd.concat(
            [
                self.df_sas_dates,
                pd.DataFrame(
                    [[np.NaN, pd.NaT, np.NaN]], columns=["date", "dtime", "time"]
                ),
            ],
            ignore_index=True,
        )
        ...
        # missing data
        pandas_missing_sav_csv = os.path.join(
            self.basic_data_folder, "sample_missing.csv"
        )
        df_missing_sav = pd.read_csv(
            pandas_missing_sav_csv, na_values="#NULL!", keep_default_na=False
        )
        df_missing_sav["mydate"] = [
            datetime.strptime(x, "%Y-%m-%d").date() if type(x) == str else float("nan")
            for x in df_missing_sav["mydate"]
        ]
        df_missing_sav["dtime"] = [
            datetime.strptime(x, "%Y-%m-%dT%H:%M:%S.000000")
            if type(x) == str
            else float("nan")
            for x in df_missing_sav["dtime"]
        ]
        df_missing_sav["mytime"] = [
            datetime.strptime(x, "%H:%M:%S.000000").time()
            if type(x) == str
            else float("nan")
            for x in df_missing_sav["mytime"]
        ]
        self.df_missing_sav = df_missing_sav

        pandas_missing_user_sav_csv = os.path.join(
            self.basic_data_folder, "sample_missing_user.csv"
        )
        df_user_missing_sav = pd.read_csv(
            pandas_missing_user_sav_csv, na_values="#NULL!", keep_default_na=False
        )
        df_user_missing_sav["mydate"] = [
            datetime.strptime(x, "%Y-%m-%d").date() if type(x) == str else float("nan")
            for x in df_user_missing_sav["mydate"]
        ]
        df_user_missing_sav["dtime"] = [
            datetime.strptime(x, "%Y-%m-%dT%H:%M:%S.000000")
            if type(x) == str
            else float("nan")
            for x in df_user_missing_sav["dtime"]
        ]
        df_user_missing_sav["mytime"] = [
            datetime.strptime(x, "%H:%M:%S.000000").time()
            if type(x) == str
            else float("nan")
        ...
        # no dates
        nodates_spss_csv = os.path.join(
            self.basic_data_folder, "sample_nodate_spss.csv"
        )
        ...
        nodates_sastata_csv = os.path.join(
            self.basic_data_folder, "sample_nodate_sas_stata.csv"
        )
        ...
        self.df_charnan = pd.DataFrame(
            [[0, np.nan, np.nan], [1, "test", timedelta]],
            columns=["integer", "string", "object"],
        )
    ...
    def test_sas7bdat_bincompressed(self):
        df, meta = pyreadstat.read_sas7bdat(
            os.path.join(self.basic_data_folder, "sample_bincompressed.sas7bdat")
        )
        self.assertTrue(df.equals(self.df_pandas))
        self.assertTrue(meta.number_columns == len(self.df_pandas.columns))
        self.assertTrue(meta.number_rows == len(self.df_pandas))
    ...
    def test_sas7bdat_metaonly(self):
        ...
        df2, meta2 = pyreadstat.read_sas7bdat(
    ...
    def test_sas7bdat_international(self):
        ...
        if sys.version_info[0] > 2:
            df, meta = pyreadstat.read_sas7bdat(
                os.path.join(self.international_data_folder, "sample.sas7bdat")
            )
            self.assertTrue(df.equals(self.df_pandas))
            self.assertTrue(meta.number_columns == len(self.df_pandas.columns))
            self.assertTrue(meta.number_rows == len(self.df_pandas))
    ...
    def test_xport_v5(self):
        df, meta = pyreadstat.read_xport(
            os.path.join(self.basic_data_folder, "sas.xpt5")
        )
        df.columns = [x.lower() for x in df.columns]
    ...
    def test_xport_v8(self):
        df, meta = pyreadstat.read_xport(
            os.path.join(self.basic_data_folder, "sas.xpt8")
        )
        self.assertTrue(df.equals(self.xptv5v8))
    ...
    def test_xport_metaonly(self):
        ...
        df2, meta2 = pyreadstat.read_xport(
            os.path.join(self.basic_data_folder, "sample.xpt"), metadataonly=True
        )
        self.assertTrue(df2.empty)
        self.assertTrue(meta.number_columns == meta2.number_columns)
    ...
    def test_xport_usecols(self):
        ...
        df, meta = pyreadstat.pyreadstat.read_xport(
            os.path.join(self.basic_data_folder, "sample.xpt"), usecols=usecols
        )
        df.columns = [x.lower() for x in df.columns]
        self.assertTrue(df.equals(self.df_usecols))
        self.assertTrue(meta.number_columns == len(self.usecols))
    ...
    def test_dta_usecols(self):
        df, meta = pyreadstat.read_dta(
            os.path.join(self.basic_data_folder, "sample.dta"), usecols=self.usecols
        )
    ...
    def test_sav_usecols(self):
        df, meta = pyreadstat.read_sav(
            os.path.join(self.basic_data_folder, "sample.sav"), usecols=self.usecols
        )
        self.assertTrue(df.equals(self.df_usecols))
        self.assertTrue(meta.number_columns == len(self.usecols))
        self.assertTrue(meta.column_names == self.usecols)
    ...
    def test_sav_missing(self):
        df, meta = pyreadstat.read_sav(
            os.path.join(self.basic_data_folder, "sample_missing.sav")
        )
        self.assertTrue(df.equals(self.df_missing_sav))
        self.assertTrue(meta.missing_ranges == {})
        df_user, meta_user = pyreadstat.read_sav(
            os.path.join(self.basic_data_folder, "sample_missing.sav"),
            user_missing=True,
        )
        self.assertTrue(df_user.equals(self.df_user_missing_sav))
        self.assertTrue(
            meta_user.missing_ranges["mynum"][0] == {"lo": 2000.0, "hi": 3000.0}
        )
        # user missing with usecols
        df_user, meta_user = pyreadstat.read_sav(
            os.path.join(self.basic_data_folder, "sample_missing.sav"),
            user_missing=True,
            usecols=["mynum", "mylabl"],
        )
    ...
    def test_zsav(self):
        df, meta = pyreadstat.read_sav(
            os.path.join(self.basic_data_folder, "sample.zsav")
        )
        self.assertTrue(df.equals(self.df_pandas))
        self.assertTrue(meta.number_columns == len(self.df_pandas.columns))
        self.assertTrue(meta.number_rows == len(self.df_pandas))
    ...
    def test_zsav_usecols(self):
        df, meta = pyreadstat.read_sav(
            os.path.join(self.basic_data_folder, "sample.zsav"), usecols=self.usecols
        )
        self.assertTrue(df.equals(self.df_usecols))
        self.assertTrue(meta.number_columns == len(self.usecols))
        self.assertTrue(meta.column_names == self.usecols)
    ...
    def test_por_usecols(self):
        df, meta = pyreadstat.read_por(
            os.path.join(self.basic_data_folder, "sample.por"), usecols=["MYNUM"]
        )
        df_pandas_por = self.df_pandas[["mynum"]]
        df.columns = [x.lower() for x in df.columns]
        self.assertTrue(df.equals(df_pandas_por))
        self.assertTrue(
            meta.number_columns == len(df_pandas_por.columns.values.tolist())
        )
    ...
    def test_sas_user_missing(self):
        ...
        unformatted_csv = os.path.join(
            self.missing_data_folder, "missing_unformatted.csv"
        )
        formatted_csv = os.path.join(
            self.missing_data_folder, "missing_sas_formatted.csv"
        )
        ...
        df_sas, meta = pyreadstat.read_sas7bdat(
            sas_file,
    ...
    def test_dta_user_missing(self):
        ...
        formatted_csv = os.path.join(
            self.missing_data_folder, "missing_dta_formatted.csv"
        )
        ...
        df_sas, meta = pyreadstat.read_dta(
            dta_file,
            apply_value_formats=True,
            user_missing=True,
            formats_as_category=False,
    ...
    def test_sav_user_missing(self):
        ...
        unformatted_csv = os.path.join(
            self.missing_data_folder, "missing_sav_unformatted.csv"
        )
        formatted_csv = os.path.join(
            self.missing_data_folder, "missing_sav_formatted.csv"
        )
        ...
        df_sas, meta = pyreadstat.read_sav(
            sav_file,
            apply_value_formats=True,
            user_missing=True,
            formats_as_category=False,
    ...
    def test_sav_missing_char(self):
        df, meta = pyreadstat.read_sav(
            os.path.join(self.missing_data_folder, "missing_char.sav")
        )
    ...
    def test_multiprocess_reader(self):
        ...
        df_multi, meta_multi = pyreadstat.read_file_multiprocessing(
            pyreadstat.read_sav, fpath
        )
    ...
    def test_chunk_reader_multiprocess(self):
        fpath = os.path.join(self.basic_data_folder, "sample_large.sav")
        reader = pyreadstat.read_file_in_chunks(
            pyreadstat.read_sav, fpath, chunksize=50, multiprocess=True
        )
        alldfs = list()
        for df, meta in reader:
            alldfs.append(df)
    ...
    def test_multiprocess_reader_xport(self):
        ...
        df_multi, meta_multi = pyreadstat.read_file_multiprocessing(
            pyreadstat.read_xport, fpath, num_rows=1000
        )
    ...
    def test_sav_write_basic(self):
        ...
        variable_value_labels = {
            "mylabl": {1.0: "Male", 2.0: "Female"},
            "myord": {1.0: "low", 2.0: "medium", 3.0: "high"},
        }
        ...
        self.assertEqual(
            meta.variable_display_width["mychar"], variable_display_width["mychar"]
        )
    ...
    def test_dta_write_basic(self):
        ...
        variable_value_labels = {
            "mylabl": {1: "Male", 2: "Female"},
            "myord": {1: "low", 2: "medium", 3: "high"},
        }
    ...
    def test_dta_write_user_missing(self):
        ...
        df_dta2, meta2 = pyreadstat.read_dta(
            path, user_missing=True, apply_value_formats=True, formats_as_category=False
        )
    ...
    def test_por_write_basic(self):
        ...
        pyreadstat.write_por(
            self.df_pandas, path, file_label=file_label, column_labels=col_labels
        )  # , note=file_note)
    ...
    def test_update_delete_file(self):
        ...
        pyreadstat.write_sav(
            df, dst_path, variable_value_labels=meta.variable_value_labels
        )
        # update
    ...
    def test_xport_write_dates2_v8(self):
        ...
        df, meta = pyreadstat.read_sas7bdat(
            os.path.join(self.basic_data_folder, "dates_xpt.sas7bdat")
        )
    ...
    def test_xport_dates2_v8(self):
        ...
        df2, meta2 = pyreadstat.read_xport(
            os.path.join(self.basic_data_folder, "dates_xpt_v8.xpt")
        )
        self.assertTrue(df.equals(df2))
    ...
    def test_sav_international_utf8_char_value(self):
        ...
        df, meta = pyreadstat.read_sav(
            os.path.join(self.basic_data_folder, "tegulu.sav")
        )
    ...
    def test_sav_international_varname(self):
        ...
        df, meta = pyreadstat.read_sav(
            os.path.join(self.basic_data_folder, "hebrews.sav")
        )
    ...
    def test_sav_original_var_types(self):
        # a file with a varname with international characters
        df, meta = pyreadstat.read_sav(
            os.path.join(self.basic_data_folder, "test_width.sav")
        )
    ...
    def test_sav_ordered_categories(self):
        ...
        df, meta = pyreadstat.read_sav(
            path, apply_value_formats=True, formats_as_ordered_category=True
        )
    ...
    def test_sav_write_pathlib(self):
        if is_pathlib_available:
            ...
            # self.assertDictEqual(meta.variable_alignment, variable_alignment)
            self.assertEqual(
                meta.variable_measure["mychar"], variable_measure["mychar"]
            )
    ...
    def test_dta_write_single_value_user_missing(self):
        ...
        pyreadstat.write_dta(
            df=df, dst_path=path, missing_user_values=missing_user_values, version=12
        )
    ...
    def test_sav_outputformat_dict(self):
        df, meta = pyreadstat.read_sav(
            os.path.join(self.basic_data_folder, "sample.sav"), output_format="dict"
        )
        self.assertTrue(meta.number_columns == len(self.df_pandas.columns))
        self.assertTrue(meta.number_rows == len(self.df_pandas))
        self.assertTrue(len(meta.notes) > 0)
